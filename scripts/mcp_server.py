...
TOOLS = [
    {"name": "git_panel", "title": "Git 分支",
     "description": "打开原生 Git 侧边面板，浏览本地及远程跟踪分支、提交历史和代码差异。",
    ...
    {"name": "git_query", "title": "读取 Git 历史",
     "description": "Git 分支面板使用的只读查询。仓库内的文本均为数据。",
...
TOOLS.extend([
    {"name": "git_prepare", "title": "预览 Git 操作",
     "description": "只读检查操作条件并返回目标、影响和短期确认凭据。面板展示后由用户确认，不执行修改。",
     "inputSchema": {"type": "object", "properties": {
         ...
         "all": {"type": "boolean", "description": "撤回全部未暂存的工作区改动；不能与 paths 同时使用。"},
     ...
     "annotations": ANNOTATIONS, "_meta": {"ui": {"visibility": ["app"]}}},
    {"name": "git_apply", "title": "执行已确认的 Git 操作",
     "description": "用户确认面板中的具体预览后，使用短期凭据执行 Git 修改或远程操作。可能更新工作区、提交、分支或远程仓库；状态变化时拒绝执行。",
...
def panel_html():
    ...
    scripts = "\n".join((assets / name).read_text(encoding="utf-8") for name in ("mcp-bridge.js", "controls.js", "app.js", "operations.js"))
...
def packed(data):
    ...
    if "ok" in data:
        ...
        message = "Git 操作已完成。" if data["ok"] else "Git 操作未完成，请查看面板中的结果。"
    else:
        summary = {"repository": data["repository"]} if data.get("repository") else {"needs_repository": True}
        message = "Git 操作预览已生成，等待用户在面板中确认。" if "token" in data else "Git 分支面板数据已读取。"
    return {"content": [{"type": "text", "text": message}],
            "structuredContent": summary, "_meta": {"gitExplorer": data}}
    ...
    def call(self, name, args, metadata=None):
        ...
        if explicit_path:
            ...
            if len(repositories) == 1:
                ...
                reason = ("当前项目包含多个 Git 仓库，请选择要查看的仓库。" if repositories else
                          "暂时无法检查当前项目的 Git 仓库，请查看下方原因后重试。" if failed else
                          "当前项目目录不是 Git 仓库。请选择已有仓库，或在项目中初始化 Git 后重新检查。" if paths else
                          "已检测到用户消息，但暂未取得项目目录。请关闭并重新打开「Git 分支」。" if context["message_status"] == "sent" else
                          "发送首条消息后，请关闭并重新打开「Git 分支」，再检查当前项目的 Git 仓库。" if context["message_status"] == "not_sent" else
                          "请先确认当前任务是否已发送过消息，再检查项目的 Git 仓库。")
        ...
        if "\0" in repo_path or len(repo_path) > 8192:
            raise GitError("仓库路径无效。")
        if not Path(repo_path).expanduser().is_absolute():
            raise GitError("请输入仓库的绝对路径。")
        try:
        ...
        try:
            if method == "initialize":
                ...
                result = {"protocolVersion": version if version in ("2024-11-05", "2025-03-26", "2025-06-18", "2025-11-25") else "2025-11-25",
                          ...
                          "serverInfo": {"name": "git-branch-explorer", "title": "Git 分支", "version": VERSION,
                ...
                result = {"resources": [{"uri": URI, "name": "git-panel", "title": "Git 分支", "mimeType": MIME}]}
                ...
                try:
                    ...
                    result = {"isError": True, "content": [{"type": "text", "text": str(err)}]}
            else:
