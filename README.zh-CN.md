...
*截图来自 v0.2.1 在本地 MCP 开发宿主中的真实运行，使用临时演示仓库 `orbit-notes`。图片展示插件面板，未包含 Codex 窗口外框。面板界面目前为中文，文档提供英文与中文两个版本。*
...
插件在能够取得项目目录时，自动打开当前任务对应的仓库，包括任务使用的 worktree。新任务尚未发送首条消息时，客户端可能还没有提供目录。面板会显示消息与项目上下文检查提示：发送消息后关闭并重新打开 **Git 分支**，或手动选择仓库。取得项目目录后，面板会检查其中的 Git 仓库，并显示具体错误。
