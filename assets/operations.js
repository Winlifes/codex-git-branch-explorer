...
window.GitOperations = (() => {
  ...
  function resultMessage(text, ok = true) {
    ...
    const close = el("button", "text-button", "关闭");
  ...
  function beginDialog(title) {
    ...
    $("confirm-operation").textContent = "预览操作";
    $("cancel-operation").textContent = "取消";
  ...
  function input(label, value = "", placeholder = "") {
    ...
    node.placeholder = placeholder; node.autocomplete = "off"; node.spellcheck = false;
  ...
  async function prepare(action, args = {}, alreadyOpen = false) {
    if (busy) return;
    if (!alreadyOpen && !beginDialog(labels[action])) return;
    const version = ++dialogVersion, path = selectedRepository;
    $("operation-error").hidden = true;
    $("confirm-operation").disabled = true;
    $("confirm-operation").textContent = "正在检查…";
    try {
      ...
      preview = data;
      $("operation-title").textContent = data.title;
      ...
      if (data.files.length) {
        target.append(el("p", "muted", data.files.length + " 个文件"));
        const list = el("div", "operation-files");
        for (const path of data.files) list.append(el("code", "", path));
        target.append(list);
      }
      $("confirm-operation").textContent = "确认" + labels[action];
  ...
  async function execute() {
    ...
    $("operation-error").hidden = true;
    $("confirm-operation").disabled = true;
    $("confirm-operation").textContent = "正在" + labels[chosen.action] + "…";
    ...
    try {
      ...
      if (chosen.action === "discard") {
        ...
        notice.replaceChildren(el("p", "", result.message + (result.ok ? " 已暂存的内容已保留。" : "")));
        if (result.backup_path) {
          const details = el("details"); details.append(el("summary", "", "查看本次撤回的本地备份"));
          details.append(el("code", "", result.backup_path),
            el("p", "muted", "manifest.json 记录原文件名；编号 .data 文件保留撤回前的原始内容。备份不会自动删除。"));
          notice.append(details);
        }
        notice.scrollIntoView({block:"nearest"});
      }
      if (!result.ok && work.open) displayError(new Error(result.message), $("work-error"));
      if (!result.ok && result.status?.conflict_count && !work.open) openWorkspace();
    } catch (err) {
      busy = false; preview = null; submit = null;
      displayError(new Error(err.message + "\n请刷新确认仓库状态后再操作；不会自动重复执行。"));
      $("confirm-operation").textContent = "请关闭后刷新";
      await loadStatus();
    } finally {
  ...
  async function network(action) {
    if (!beginDialog(labels[action] + "远程分支")) return;
    const version = dialogVersion;
    await loadStatus();
    if (!dialog.open || version !== dialogVersion) return;
    if (!current?.remotes.length) {
      displayError(new Error("当前仓库还没有配置远程地址，请先在终端添加远程仓库，再刷新。")); return;
    }
    ...
    const showAddress = () => {
      ...
      path.textContent = (action === "push" ? r.push_urls : r.fetch_urls).join("\n");
    };
    select.addEventListener("change", showAddress); showAddress();
    let branch, upstream;
    if (action !== "fetch") {
      branch = input(action === "push" ? "远程目标分支" : "拉取的远程分支",
        (current.upstream?.ref || current.repository.current_ref || "").replace(/^refs\/heads\//, ""));
      if (action === "push") upstream = checkbox("设为当前分支的上游", !current.upstream);
    }
  ...
  function createBranch() {
    ...
    if (!beginDialog("新建分支")) return;
    $("operation-content").append(el("p", "operation-description", "从 " + (state.branch?.name || "当前提交") + " 创建本地分支。"));
    const name = input("分支名称", "", "feature/my-change");
    name.maxLength = 240;
    const checkout = checkbox("创建后切换到新分支", true);
  ...
  function addRevertButton(target, commit) {
    ...
    const button = el("button", "button revert-button", "撤销此提交…");
    button.addEventListener("click", () => {
      if (commit.parents.length < 2) { prepare("revert", {sha: commit.sha}); return; }
      if (!beginDialog("撤销合并提交")) return;
      $("operation-content").append(el("p", "operation-description", "选择保留的父提交主线。将创建反向提交，撤销相对该主线引入的改动。"));
      const select = el("select");
      commit.parents.forEach((sha, i) => {
        const option = el("option", "", "父提交 " + (i + 1) + " · " + sha.slice(0, 12));
  ...
  async function openWorkspace() {
    ...
    $("work-changes").replaceChildren(el("p", "muted", "正在读取工作区…"));
  ...
  function renderWorkspace() {
    const s = current;
    if (!s) return;
    $("work-branch").textContent = s.repository.name + " / " +
      (s.repository.current_ref?.replace(/^refs\/heads\//, "") || "游离 HEAD");
    ...
    if (s.operation) {
      const title = s.operation === "merge" ? "合并" : s.operation === "revert" ? "撤销" : s.operation;
      progress.append(el("strong", "", title + "尚未完成"), el("p", "", s.conflict_count ?
        s.conflict_count + " 个冲突文件。请在编辑器中解决后，回到这里暂存并继续。" : "请检查暂存内容，确认后继续完成操作。"));
      if (["merge", "revert"].includes(s.operation)) {
        const actions = el("div", "git-actions");
        for (const action of ["continue", "abort"]) {
          const button = el("button", "button", (action === "continue" ? "继续" : "中止") + title);
    ...
    for (const [key, title, action] of [["unstaged", "更改", "stage"], ["staged", "已暂存", "unstage"]]) {
      ...
      heading.append(el("h3", "", title + " · " + files.length));
      const paths = files.filter(c => c.operable).map(c => c.path);
      const all = el("button", "text-button", action === "stage" ? "全部暂存" : "全部取消暂存");
      ...
      if (key === "unstaged") {
        const discard = el("button", "text-button discard-all", "全部撤回");
        discard.prepend(window.GitUI.icon("undo"));
        discard.title = "撤回全部未暂存的改动，保留已暂存的内容";
      ...
      if (!files.length) group.append(el("p", "work-empty", key === "unstaged" ? "工作区没有未暂存的更改" : "还没有暂存的更改"));
      for (const file of files) {
        const row = el("div", "work-file");
        const view = el("button", "work-file-path");
        view.append(el("span", "file-status" + (file.conflict ? " del" : ""), file.conflict ? "冲突" :
        ...
        change.setAttribute("aria-label", (action === "stage" ? "暂存 " : "取消暂存 ") + file.path);
        ...
        if (key === "unstaged") {
          ...
          discard.setAttribute("aria-label", "撤回改动 " + file.path);
          discard.title = "撤回此文件未暂存的改动，保留已暂存的内容";
    ...
    $("commit-hint").textContent = "将提交 " + s.staged_count + " 个已暂存文件";
  ...
  async function showWorkingPatch(path, staged) {
    $("work-patch-path").textContent = path + (staged ? " · 已暂存" : " · 工作区");
    $("work-patch-section").hidden = false;
    $("work-patch").textContent = "正在读取差异…";
    try {
      ...
      if (data.truncated) fragment.append(el("span", "diff-line meta", "差异较大，仅展示前 800 KB。"));
